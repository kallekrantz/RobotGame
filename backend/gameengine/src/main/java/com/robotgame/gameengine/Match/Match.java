...
public class Match implements Runnable
{
    ...
    public boolean BuildRobots(Vector<RobotBlueprint> blueprints)
    {
        ...
        for (int n = 0; n < _numRobots; n++)
        {
            ...
            _robots[n].SetStartPos(new Vector2(-3 + 6 * n, 0));
...
//      8. Advance the game clock.
    ...
    private void CalcCollisions()
    {
        ...
        for (int n = 0; n < _numRobots; n++)
        {
            ...
            if (pos.x >  3) { _robots[n].GetCurrentState().pos.x =  3; _robots[n].GetCurrentState().vel.x = 0; }
            if (pos.x < -3) { _robots[n].GetCurrentState().pos.x = -3; _robots[n].GetCurrentState().vel.x = 0; }
            if (pos.y >  3) { _robots[n].GetCurrentState().pos.y =  3; _robots[n].GetCurrentState().vel.y = 0; }
            if (pos.y < -3) { _robots[n].GetCurrentState().pos.y = -3; _robots[n].GetCurrentState().vel.y = 0; }
